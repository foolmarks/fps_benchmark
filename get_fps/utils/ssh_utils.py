...
def create_forward_tunnel(args, password: str, max_attempts: int):
    ...
    for attempt in range(max_attempts):
        if password == '':
            ...
            cmd = ["sshpass", "-p", "{}".format(password), "ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", "-f", "-N", "-L", f"{local_port}:localhost:{args.dv_port} {args.dv_user}@{args.dv_host}"]
        cmd = ' '.join(cmd)
...
def create_forward_tunnel_generic(dv_port, dv_user, dv_host, password: str, max_attempts: int):
    ...
    for attempt in range(max_attempts):
        if password == '':
            ...
            cmd = ["sshpass", "-p", "{}".format(password), "ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", "-f", "-N", "-L", f"{local_port}:localhost:{dv_port} {dv_user}@{dv_host}"]
        cmd = ' '.join(cmd)
...
def scp_files_to_remote(args, file_path, password: str, dest: str, max_attempts: int, dv_user: str = '', dv_host: str = ''):
    ...
    scp_string = f"scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null{' -O' if '9.0' in version_string else ''}"
    for attempt in range(max_attempts):
        if password == '':
...
def delete_files_from_remote(args, file_name: str, password: str, dest: str, dv_user: str = '', dv_host: str = ''):
    """
        Delete the files from Remote

        :param args: ArgumentParser.
        :param file_name: File name to be deleted
        :param password: Password for the SSH user
        :param dest: target directory.
        :param dv_user: UserName for the SSH user
        :param dv_host: IP of the host machine.
        """
    if not args:
        assert dv_user != '' and dv_host != '', "Please provide username and hostname"
        devkit_username = dv_user
        ...
        devkit_username = args.dv_user
        devkit_hostname = args.dv_host
    output = subprocess.check_output(['ssh', '-V'],
                                     stderr=subprocess.STDOUT)
    version_line = output.decode().strip().splitlines()[0]
    version_string = version_line.split(' ')[0]
    ## If Mac OS is Ventura (version:13.2.1), and Openssh is OpenSSH_9.0p1 or later. Need to add -O due to https://www.openssh.com/txt/release-9.0.
    ssh_string = f"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null{' -O' if '9.0' in version_string else ''}"
    if password == '':
        del_cmd = [ssh_string, f'{devkit_username}@{devkit_hostname} "rm -rf {dest}/{file_name}"']
    else:
        del_cmd = ["sshpass", "-p", "{}".format(password), ssh_string,
                   f'{devkit_username}@{devkit_hostname} "rm -rf {dest}/{file_name}"']
    scp_cmd = ' '.join(del_cmd)
    scp_connection = subprocess.Popen(scp_cmd, stdin=subprocess.PIPE,
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE, shell=True)
    output, error = scp_connection.communicate()
    if scp_connection.returncode != 0:
        logging.debug(f'Failed to delete files from {dest}')
        logging.debug(f'ERROR {error}')
    else:
        logging.debug(
            f'Files from {dest} successfully deleted with pid {scp_connection.pid}')
...
def run_remote_command(args: str, command: str, password: str, max_attempts: int, command_list=None):
    ...
    for attempt in range(max_attempts):
        if password != '':
            remote_command = ["sshpass", "-p", "{}".format(password), "ssh", "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", f"{args.dv_user}@{args.dv_host}", f"{command}"]
